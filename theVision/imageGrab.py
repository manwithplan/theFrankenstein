...
the imageGrab module is used to grab images from the screen, to be formatted and sent through the neural network for a prediction.
findScreen class is used for testing, as it finds a video window that hasn't been set before. In production we will know the name 
of the window from which to take a screenShot, and that will be a different class.
...
class findScreen():  
    ...
    def trackWindow(self, avg):
        ...
        if contours:
            ...
            accuracyCoordinates = (self.optimalCoordinates['x'][1] + self.optimalCoordinates['y'][1] + self.optimalCoordinates['w'][1] + self.optimalCoordinates['h'][1]) / 4
