...
from Settings.Settings import coordinatesHUD, coordinatesDocking, coordinatesConflict, coordinatesSpeed, coordinatesPlanet, coordinatesShields, coordinatesScaler, coordinatesMiniMap, coordinatesSupercruise
...
class detectionMain():
    ...
    def stateLogic(self):
        ...
        if self.detectedHUD:
            ...
            if self.detectedConflict == True and self.detectedSupercruise == False:
    ...
    def runDetection(self, inputs, timeIt = False):
        ...
        _, screenAngle = self.detectionHUD( self.cropWindow(self.inputGreen, coordinatesHUD ) )
        ...
        if self.detectedHUD:
            ...
            if timeIt:
                ...
                _ = self.detectionDocking( self.cropWindow(self.inputBlue, coordinatesDocking ) )
                dockingTime = time.time() - start_time

                start_time = time.time()
                # running conflict detection algo w time:
                _ = self.detectionConflict( self.cropWindow(self.inputRed, coordinatesConflict ) )
                conflictTime = time.time() - start_time

                start_time = time.time()
                # running speed detection algo w time:
                _ = self.detectionSpeed( self.cropWindow(self.inputGreen, coordinatesSpeed) )
                speedTime = time.time() - start_time

                start_time = time.time()
                # running proximity to planet detection algo w time:
                _ = self.detectionPlanet( self.cropWindow(self.inputGreen, coordinatesPlanet) )
                planetTime = time.time() - start_time

                start_time = time.time()
                # running proximity to shield detection algo w time:
                _ = self.detectionShields( self.cropWindow(self.inputBlue, coordinatesShields) )
                shieldsTime = time.time() - start_time

                start_time = time.time()
                # running proximity to minimap detection algo w time:
                _ = self.detectionMiniMap( [self.cropWindow(self.inputRed, coordinatesMiniMap),  self.cropWindow(self.inputWhite, coordinatesMiniMap)] )
                miniMapTime = time.time() - start_time

                start_time = time.time()
                # running proximity to supercruise detection algo w time:
                _ = self.detectionSupercruise( self.cropWindow(self.inputGray, coordinatesSupercruise))
        ...
        if timeIt:
            totalTime = HUDTime + rotationTime + conflictTime + speedTime + planetTime + shieldsTime + miniMapTime + supercruiseTime
            print(' \n state : {} - total time : {:.3f} \n HUD : {} in {:.3f}s \n rotation in {:.3f}s \n docking : {} in {:.3f}s \n conflict : {} in {:.3f}s \n speed : {:.2f} in {:.3f}s \n planet : {} in {:.3f}s \n shields : {:.2f} in {:.3f}s \n hostiles : {} in {:.3f}s \n supercruise : {} in {:.3f}s'.format(self.gameStateList, totalTime, self.detectedHUD, HUDTime, rotationTime, self.detectedDocking, dockingTime, self.detectedConflict, conflictTime, self.detectedSpeed, speedTime, self.detectedPlanet, planetTime, self.detectedShields, shieldsTime, self.detectedHostiles, miniMapTime, self.detectedSupercruise, supercruiseTime), end="\r")
            ...
            if debugMode:
                if lines is not None:
                    for i in range(0, len(lines)):
                        l = lines[i][0]
                        cv2.line(inputImage, (l[0], l[1]), (l[2], l[3]), (255,255,255), 1, cv2.LINE_AA)
