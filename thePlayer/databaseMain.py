...
from Settings.Settings import rootURL, musicFullDataBaseURL, musicSnippetsDataBaseURL, musicMoodsDataBaseURL, shutterSpeed
...
class databaseMain():
    ...
    def gatherSnippets(self, match):
        ...
        for number in range(lookupKey, total):
            matchSecondaryKey = f"{int(match['PrimaryKeys'].item())}_{str(number).zfill(4)}"
            snippetToWrite = self.df_snippets.loc[self.df_snippets['SecondaryKeys'] == matchSecondaryKey]
