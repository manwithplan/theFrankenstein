...
from Settings.Settings import rootURL, musicFullURL, musicSnippetsURL, shutterSpeed
...
class playerStream():
    ...
    def __init__(self):
        ...
        self.segment = pydub.AudioSegment.silent(duration= shutterSpeed * 1000, frame_rate=self.rate).set_channels(2)
        ...
        self.pipeline = [pydub.AudioSegment.silent(duration = shutterSpeed * 1000, frame_rate=self.rate).set_channels(2)]
        self.pipelineNames = []
    ...
    def fadeAndMix(self, fileNames, location = 0):
        ...
        try:
            ...
            if len(self.pipeline[-1]) >= len( firstSnippet ):
                ...
                self.pipeline[-1] = self.pipeline[-1].append( firstSnippet, crossfade = len(self.pipeline[-1]) - 100 )
